...
if(isset($_FILES['itemImage'])){
	$itemImage = $target_dir . (time()+1000) .	 "-" . preg_replace('/\s+/', '_', $_FILES['itemImage']['name']);
...
$stmt = $conn->prepare($sql);
$stmt->bind_param('sssssiiisssisi', $itemBarcode, $itemName, $itemBrand, $itemSize, $itemColor, $itemQuantity, $itemPurchase, $itemSell, $itemExpiry, $itemImage, $itemDescription, $warehouse, $_COOKIE["US-K"], $categoryId);
if($stmt->execute()){
	$stmt->close();
	die("Success");
}
