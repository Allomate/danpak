...
if ($inventoryLocation == "true") {
	$sql = "SELECT item_id, item_barcode, item_name, item_size, item_color, item_quantity, created_at from location_based_inventory where franchise_id = (SELECT franchise_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?)) order by item_name";	
}
