...
foreach ($csvAsArray as $value) {
	...
	$sql = "SELECT warehouse_id from warehouse where lower(warehouse_name) = lower(?) and company_id = (SELECT company_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?))";
	$stmt = $conn->prepare($sql);
	...
	if (!$stmt->fetch()) {
		$errorsThrown[] = array(
			'barcode'=> $barcode,
			'error'=> 'Warehouse information is incorrect'
			);
		continue;
	}
	$stmt->close();
