...
if($isDeal){
	...
	$stmt = $conn->prepare($sql);
	$stmt->bind_param('iiiiiis', $complainId, $_COOKIE["US-K"], $complainId, $complainId, $discount, $reward, $product);
	if(!$stmt->execute())
		die(htmlspecialchars($stmt->error));
	$stmt->close();
    ...
    // echo $complainId,"<br>";
    // die;
    // $sql = "INSERT into push_notifications_log(`user_id`, `company_id`, `complain_id`) values(?,?,?)";
    // $stmt=$conn->prepare($sql);
    // $stmt->bind_param('iii', $userId, $companyId, $complainId);
    // $stmt->execute();
    // $stmt->close();
