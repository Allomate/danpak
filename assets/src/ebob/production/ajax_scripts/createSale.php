...
for ($i=0; $i < sizeof(json_decode($data, true)); $i++) { 
	...
	if ($sold_by == "none" || $sold_by == "") {
		$sql = "INSERT INTO `sales_management`(`item_id`, `item_quantity`, `item_price_each`, `item_final_price`, `item_discount`, `sold_to`, `sold_by`, `sale_token`, `payment_method`, `franchise_id`, `sold_at`) VALUES (?,?,?,?,?,?,(SELECT employee_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?)),?,?,(SELECT franchise_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?)) ,?)";
		$stmt = $conn->prepare($sql);
		$stmt->bind_param('iiiiissssss', $item_id, $item_quantity, $item_price_each, $item_final_price, $item_discount, $sold_to, $_COOKIE["US-K"], $token, $payment_method, $_COOKIE['US-K'], $sold_at);
		if($stmt->execute()){
			$stmt->close();
		...
		$sql = "INSERT INTO `sales_management`(`item_id`, `item_quantity`, `item_price_each`, `item_final_price`, `item_discount`, `sold_to`, `sold_by`, `sale_token`, `payment_method`, `franchise_id`, `sold_at`) VALUES (?,?,?,?,?,?,?,?,?,(SELECT franchise_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?)) ,?)";
		$stmt = $conn->prepare($sql);
		$stmt->bind_param('iiiiisissss', $item_id, $item_quantity, $item_price_each, $item_final_price, $item_discount, $sold_to, $sold_by, $token, $payment_method, $_COOKIE['US-K'], $sold_at);
		if($stmt->execute()){
			$stmt->close();
			$sql = "UPDATE location_based_inventory set item_quantity = item_quantity - ? where item_id = ?";
			$stmt = $conn->prepare($sql);
