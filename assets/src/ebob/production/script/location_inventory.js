$(document).ready(function(){
	...
	$(document).on('click', '.deleteInventButton', function(e) {
		...
		if ($(this).parent().parent().find("#discountInput").val() == "" || parseInt($(this).parent().parent().find("#discountInput").val()) < 0) {
			swal('Invalid Discount', 'Please enter valid amount of discount to apply', 'warning');
			return;
		}

		var ajaxData = {'id': $(this).attr('id'), 'discount' : $(this).parent().parent().find("#discountInput").val()};
			...
			preConfirm: function () {
				return new Promise(function (resolve, reject) {
					// ajaxer('deleteThisLocationInventItem.php', ajaxData, function(response){
						ajaxer('applyDiscountLocationInvent.php', ajaxData, function(response){
							if (response == "Success") {
								swal('Discount Applied', 'Discount has been applied successfully', 'success');
								fetchAcquiredInventory(null);
							}else{
								swal('Failed', 'Unable to apply discount at the moment', 'error');
							}
							return;
						});
					})
...
function fetchAcquiredInventory(target){
	...
	ajaxer('getAllInventory.php', ajaxData, function(response){
		...
		for (var i = 0; i < data.length; i++) {
			$('#inventoryTbody').append('<tr><td>'+data[i]["barcode"]+'</td><td>'+data[i]["name"]+'</td><td>'+data[i]["color"]+'</td><td>'+data[i]["size"]+'</td><td>'+data[i]["quantity"]+'</td><td>'+data[i]["createdOn"]+'</td><td><input type="number" class="form-control" id="discountInput" value="'+data[i]["discount_available"]+'" /></td><td id="requestInventTd"><button class="btn btn-info reqInventButton" id="'+data[i]["id"]+'" style="font-size: 12px;"><img src="assets/raw/view-details-loader.gif" class="requestInventSpinner"><span id="reqInventText">Request Inventory</span></button>&nbsp;&nbsp;&nbsp; <button class="btn btn-success deleteInventButton" id="'+data[i]["id"]+'" style="font-size: 12px;"><img src="assets/raw/view-details-loader.gif" class="deleteInventSpinner"><span id="deleteInventText">Apply Discount</span></button></td></tr>');
		}
		if (target) {
