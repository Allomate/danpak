...
<div id="page-container" class="fade page-sidebar-fixed page-header-fixed">
 ...
 <div id="content" class="content">
    ...
    <!-- end breadcrumb -->
    <!-- begin page-header -->
    <h1 class="page-header">Warehouse Gallery <small>You can view all of the inventory from this page</small></h1>
    <!-- end page-header -->
    <div id="gallery" class="gallery">
        <?php
        require_once 'database/config.php';
        $sql = "SELECT `item_id`, `item_barcode`, `item_name`, `item_brand`, `item_image`, `item_description` FROM `warehouse_inventory` wi WHERE company_id = (SELECT company_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?))";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('s', $_COOKIE["US-K"]);
        $stmt->execute();
        $stmt->bind_result($itemId, $barcode, $name, $brand, $image, $description);
        $galleryGroupCounter = 1;
        while ($stmt->fetch()) {?>
        <div class="image gallery-group-<?php echo $galleryGroupCounter;?>">
            <div class="image-inner">
                <a href="<?php echo $image!='' ? $image : 'uploads/item_images/no-preview.jpg';?>" data-lightbox="gallery-group-<?php echo $galleryGroupCounter;?>">
                    <?php
                    if (!$image) {?>
...
<script type="text/javascript" src="script/warehouse_gallery_script.js?v=<?php echo time();?>"></script>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#warehouseInventGallery').addClass('active');
    });
</script>