<div id="sidebar" class="sidebar">
    ...
    <div data-scrollbar="true" data-height="100%">
        ...
        $sql = "SELECT `id`, `employee_id`, `cx_dashboard`, `add_company`, `add_franchise`, `update_company`, `add_employee`, `view_employee`, `inbox`, `complain_heads`, `warehouse_inventory`, `location_inventory`, `inventory_requests`, `add_warehouse`, `dispatch_items`, `add_items`, `update_items`, `item_sale`, `pos_setup`, `add_targets`, `update_targets`, `permissions`, `update_locations`, `warehouse_inventory_gallery`, `location_inventory_gallery`, `customer_base` FROM `access_rights` WHERE employee_id = (SELECT employee_id from employees_info where employee_username = (SELECT employee from employee_session where session = ?))";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('s', $_COOKIE["US-K"]);
        $stmt->execute();
        $stmt->bind_result($id, $employeeId, $dashboard, $add_company, $add_franchise, $update_company, $add_employee, $view_employee, $inbox, $complain_heads, $warehouse_inventory, $location_inventory, $inventory_requests, $add_warehouse, $dispatch_items, $add_items, $update_items, $item_sale, $pos_setup, $add_targets, $update_targets, $access_rights, $update_locations, $warehouse_inventory_gallery, $location_inventory_gallery, $customer_base);
        while ($stmt->fetch()) {?>
        <ul class="nav">
            <li class="nav-header">Navigation</li>
            <li id="dashboard"><a href="dashboardv2.php"><i class="fa fa-sign-out"></i>Dashboard V2</a></li>
            <?php
            if ($dashboard) {?>
            ...
            <li class="has-sub" id="companyDetailsLi">
                ...
                <ul class="sub-menu">
                    ...
                    <?php }
                    if ($add_warehouse) {?>
                    <li id="addWarehouse"><a href="add_warehouse.php">Add Warehouse</a></li>
                    <?php }
                    if ($add_company) {?>
