...
class RetailerInformation extends Web_Services_Controller
{
    ...
    {
        $retailerInfo = (array) json_decode(file_get_contents('php://input'), true);
        if ($this->AuthenticateWebServiceCall($retailerInfo["api_secret_key"])):
            unset($retailerInfo['api_secret_key']);
            $counter = 0;
            $config['upload_path'] = './assets/uploads/retailers/';
            foreach ($retailerInfo["retailers"] as $ret) {
                if (isset($ret['retailer_image_b64']) && $ret['retailer_image_b64'] != ''):
                    $imgData = base64_decode($ret['retailer_image_b64']);
                    $imageName = $config['upload_path'] . random_string('alnum', 10) . '_' . time() . '.jpg';
                    $ifp = fopen($imageName, 'wb');
                    fwrite($ifp, $imgData);
                    fclose($ifp);
                	unset($retailerInfo["retailers"][$counter]['retailer_image_b64']);
                    $retailerInfo["retailers"][$counter]['retailer_image'] = $imageName;
                endif;
                if (isset($ret['retailer_image_b64']) && $ret['retailer_image_b64'] == ''):
		                unset($retailerInfo["retailers"][$counter]['retailer_image_b64']);
                endif;
