...
class WebServices extends CI_Model
{
    ...
    public function DoneVisits($userInfo)
    {
        ...
        if ($this->db->select('GROUP_CONCAT(retailer_id) as retailer_ids')->where('employee_id', $employee_id)->get('retailers_assignment')->row()):
            $retailersAssigned = $this->db->select('GROUP_CONCAT(retailer_id) as retailer_ids')->where('employee_id', $employee_id)->get('retailers_assignment')->row()->retailer_ids;
            if ($this->db->select('GROUP_CONCAT(retailer_id) as retailers')->where('employee_id = ' . $employee_id . ' and DATE(plan_for_day) = CURDATE() and find_in_set(retailer_id, "' . $retailersAssigned . '")')->group_by('DATE(plan_for_day)')->get('employee_visit_plan')->row()):
                $retailersListFromPlanToday = $this->db->select('GROUP_CONCAT(retailer_id) as retailers')->where('employee_id = ' . $employee_id . ' and DATE(plan_for_day) = CURDATE() and find_in_set(retailer_id, "' . $retailersAssigned . '")')->group_by('DATE(plan_for_day)')->get('employee_visit_plan')->row()->retailers;
                if ($this->db->where('find_in_set(retailer_id, "' . $retailersListFromPlanToday . '") and employee_id = ' . $employee_id . ' and DATE(created_at) = CURDATE()')->get('visits_marked')->result()):
                    $retailersList = $this->db->select('GROUP_CONCAT(retailer_id) as retailersList')->where('find_in_set(retailer_id, "' . $retailersListFromPlanToday . '") and employee_id = ' . $employee_id . ' and DATE(created_at) = CURDATE()')->get('visits_marked')->row()->retailersList;
                ...
                return "No retailers in plan today";
            endif;
        else:
