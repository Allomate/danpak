...
class OrdersModel extends CI_Model{
		...
		if(!$status || $status === "pending"){
			...
			foreach($dates as $today):
				for($i = 0; $i < sizeOf($emps); $i++){
					$ordersPrice = $this->db->select('SUM(final_price) as final_price')->where('order_id IN (SELECT id from orders where employee_id = '.$emps[$i]->employee_id.' and (LOWER(status) = "'.$status.'" OR status IS NULL or status = "") and DATE(created_at) = "'.$today->date.'" )')->get('order_contents')->row()->final_price;
			...
			foreach($dates as $today):
				for($i = 0; $i < sizeOf($emps); $i++){
					...
					if($ordersCount){
						$ordersPrice = $this->db->select('SUM(final_price) as final_price')->where('order_id IN (SELECT id from orders where employee_id = '.$emps[$i]->employee_id.' and LOWER(status) = "'.$status.'" and DATE(created_at) = "'.$today->date.'" )')->get('order_contents')->row()->final_price;
	...
	public function getAllOrders($employee, $date, $status){
		$orderDetails = $this->db->select('id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username, (SELECT territory_name from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)) as territory, (SELECT area_name from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id))) as area, (SELECT region_name from regions_info where id = (SELECT region_id from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)))) as region, employee_id, (SELECT retailer_name from retailers_details where id = orders.retailer_id) as distributor_name, (SELECT retailer_lats from retailers_details where id = orders.retailer_id) as retailer_lats, (SELECT retailer_longs from retailers_details where id = orders.retailer_id) as retailer_longs, retailer_id as distributor_id, within_radius, status,booker_lats, booker_longs, DATE(created_at) as created_at, (SELECT SUM(final_price) from order_contents where order_id = orders.id) as final_price')->where('DATE(created_at) = "'.$date.'" and LOWER(status) = "'.$status.'" and employee_id = '.$employee)->get("orders")->result();
		if (!$status || $status === "pending") :
			$orderDetails = $this->db->select('id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username, (SELECT territory_name from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)) as territory, (SELECT area_name from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id))) as area, (SELECT region_name from regions_info where id = (SELECT region_id from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)))) as region, employee_id, (SELECT retailer_name from retailers_details where id = orders.retailer_id) as distributor_name, (SELECT retailer_lats from retailers_details where id = orders.retailer_id) as retailer_lats, (SELECT retailer_longs from retailers_details where id = orders.retailer_id) as retailer_longs, retailer_id as distributor_id, booker_lats, status, booker_longs, within_radius, DATE(created_at) as created_at, (SELECT SUM(final_price) from order_contents where order_id = orders.id) as final_price')->where('employee_id = '.$employee.' and DATE(created_at) = "'.$date.'" and (status is NULL or status = "pending" or status = "")')->get("orders")->result();
			$orderStats = array('Pending'=>$this->db->select('count(*) as pendingOrders')->where('status is null or status = "pending" or status = ""')->get('orders')->row()->pendingOrders,'Total'=>$this->db->select('count(*) as totalOrders')->get('orders')->row()->totalOrders,'Cancelled'=>$this->db->select('count(*) as cancelledOrders')->where('status','cancelled')->get('orders')->row()->cancelledOrders, 'Completed'=>$this->db->select('count(*) as completedOrders')->where('status','completed')->get('orders')->row()->completedOrders, 'Compliance'=>$this->db->select('count(*) as compliance')->where('within_radius', 1)->get('orders')->row()->compliance, 'NonCompliance'=>$this->db->select('count(*) as non_compliance')->where('within_radius', 0)->get('orders')->row()->non_compliance);
			return array('stats'=>$orderStats, 'orderDetails'=>$orderDetails);
			elseif($status == "latest") :
			$orderDetails = $this->db->select('id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username, (SELECT territory_name from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)) as territory, (SELECT area_name from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id))) as area, (SELECT region_name from regions_info where id = (SELECT region_id from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)))) as region, employee_id, (SELECT retailer_name from retailers_details where id = orders.retailer_id) as distributor_name, (SELECT retailer_lats from retailers_details where id = orders.retailer_id) as retailer_lats, (SELECT retailer_longs from retailers_details where id = orders.retailer_id) as retailer_longs, retailer_id as distributor_id, within_radius, status, booker_lats, booker_longs,DATE(created_at) as created_at, (SELECT SUM(final_price) from order_contents where order_id = orders.id) as final_price')->where('DATE(created_at) = CURDATE() and employee_id = '.$employee)->order_by("(SELECT employee_username from employees_info where employee_id = orders.employee_id)")->get("orders")->result();
			elseif($status == "employee") :
			$orderDetails = $this->db->select('id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username, (SELECT territory_name from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)) as territory, (SELECT area_name from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id))) as area, (SELECT region_name from regions_info where id = (SELECT region_id from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)))) as region, employee_id, (SELECT retailer_name from retailers_details where id = orders.retailer_id) as distributor_name, (SELECT retailer_lats from retailers_details where id = orders.retailer_id) as retailer_lats, (SELECT retailer_longs from retailers_details where id = orders.retailer_id) as retailer_longs, retailer_id as distributor_id, within_radius, status, booker_lats, booker_longs,DATE(created_at) as created_at, (SELECT SUM(final_price) from order_contents where order_id = orders.id) as final_price')->order_by("(SELECT employee_username from employees_info where employee_id = orders.employee_id)")->get("orders")->result();
		endif;
		$orderStats = array('Pending'=>$this->db->select('count(*) as pendingOrders')->where('status is null or status = "pending" or status = ""')->get('orders')->row()->pendingOrders,'Total'=>$this->db->select('count(*) as totalOrders')->get('orders')->row()->totalOrders,'Cancelled'=>$this->db->select('count(*) as cancelledOrders')->where('status','cancelled')->get('orders')->row()->cancelledOrders, 'Completed'=>$this->db->select('count(*) as completedOrders')->where('status','completed')->get('orders')->row()->completedOrders, 'Compliance'=>$this->db->select('count(*) as compliance')->where('within_radius', 1)->get('orders')->row()->compliance, 'NonCompliance'=>$this->db->select('count(*) as non_compliance')->where('within_radius', 0)->get('orders')->row()->non_compliance);
		return array('stats'=>$orderStats, 'orderDetails'=>$orderDetails);
	}
