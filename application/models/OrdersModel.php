	...
	public function getCollectiveOrders($status){
		...
		if($this->session->userdata("user_type") == "danpak"){
			if(!$status || $status === "pending"){
				...
				endforeach;
				$orderStats = array('Pending'=>$this->db->select('count(*) as pendingOrders')->where('status is null or status = "pending" or status = ""')->get('orders')->row()->pendingOrders,'Total'=>$this->db->select('count(*) as totalOrders')->get('orders')->row()->totalOrders,'Cancelled'=>$this->db->select('count(*) as cancelledOrders')->where('status','cancelled')->get('orders')->row()->cancelledOrders, 'Completed'=>$this->db->select('count(*) as completedOrders')->where('status','completed')->get('orders')->row()->completedOrders, 'Compliance'=>$this->db->select('count(*) as compliance')->where('within_radius', 1)->get('orders')->row()->compliance, 'NonCompliance'=>$this->db->select('count(*) as non_compliance')->where('within_radius', 0)->get('orders')->row()->non_compliance, "orders_price" => $ordersPrice);
			}else{
			...
			if(!$status || $status === "pending"){
				$dates = $this->db->select('DATE(created_at) as date')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and (LOWER(status) = "'.$status.'" OR status IS NULL or status = "")')->group_by('DATE(created_at)')->get('orders')->result();
				$emps = $this->db->select('employee_id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and (LOWER(status) = "'.$status.'" OR status IS NULL or status = "")')->group_by('employee_id')->get('orders')->result();
				...
				endforeach;
				$orderStats = array('Pending'=>$this->db->select('count(*) as pendingOrders')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and (status is null or status = "pending" or status = "")')->get('orders')->row()->pendingOrders,'Total'=>$this->db->select('count(*) as totalOrders')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->totalOrders,'Cancelled'=>$this->db->select('count(*) as cancelledOrders')->where('status = "cancelled" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->cancelledOrders, 'Completed'=>$this->db->select('count(*) as completedOrders')->where('status = "completed" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->completedOrders, 'Compliance'=>$this->db->select('count(*) as compliance')->where('within_radius = 1 and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->compliance, 'NonCompliance'=>$this->db->select('count(*) as non_compliance')->where('within_radius = 0 and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->non_compliance);
			}else if($status == "completed" || $status == "processed" || $status == "cancelled"){
				$dates = $this->db->select('DATE(created_at) as date')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and LOWER(status) = "'.$status.'"')->group_by('DATE(created_at)')->get('orders')->result();
				$emps = $this->db->select('employee_id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username')->where('LOWER(status) = "'.$status.'" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->group_by('employee_id')->get('orders')->result();
				$orderDetails = array();
				$ordersPrice = array();
				foreach($dates as $today):
					for($i = 0; $i < sizeOf($emps); $i++){
				...
				endforeach;
				$orderStats = array('Pending'=>$this->db->select('count(*) as pendingOrders')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and (status is null or status = "pending" or status = "")')->get('orders')->row()->pendingOrders,'Total'=>$this->db->select('count(*) as totalOrders')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->totalOrders,'Cancelled'=>$this->db->select('count(*) as cancelledOrders')->where('status = "cancelled" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->cancelledOrders, 'Completed'=>$this->db->select('count(*) as completedOrders')->where('status = "completed" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->completedOrders, 'Compliance'=>$this->db->select('count(*) as compliance')->where('within_radius = 1 and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->compliance, 'NonCompliance'=>$this->db->select('count(*) as non_compliance')->where('within_radius = 0 and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->non_compliance, "orders_price" => $ordersPrice);
			}else{
				$dates = $this->db->select('DATE(created_at) as date')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and DATE(created_at) = CURDATE()')->group_by('DATE(created_at)')->get('orders')->result();
				$emps = $this->db->select('employee_id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username')->where('DATE(created_at) = CURDATE() and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->group_by('employee_id')->get('orders')->result();
				$orderDetails = array();
				foreach($dates as $today):
					for($i = 0; $i < sizeOf($emps); $i++){
						$ordersCount = $this->db->select('count(*) as totalOrders')->where('DATE(created_at) = "'.$today->date.'" and employee_id = '.$emps[$i]->employee_id)->get('orders')->row()->totalOrders;
	...
	public function getAllOrders($employee, $date, $status){
		if($this->session->userdata("user_type") == "danpak"){
			...
			if (!$status || $status === "pending") :
				...
				return array('stats'=>$orderStats, 'orderDetails'=>$orderDetails);
			elseif($status == "latest") :
				$orderDetails = $this->db->select('id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username, (SELECT territory_name from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)) as territory, (SELECT area_name from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id))) as area, (SELECT region_name from regions_info where id = (SELECT region_id from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)))) as region, employee_id, (SELECT retailer_name from retailers_details where id = orders.retailer_id) as distributor_name, (SELECT retailer_lats from retailers_details where id = orders.retailer_id) as retailer_lats, (SELECT retailer_longs from retailers_details where id = orders.retailer_id) as retailer_longs, retailer_id as distributor_id, within_radius, status, booker_lats, booker_longs,DATE(created_at) as created_at, (SELECT SUM(final_price) from order_contents where order_id = orders.id and item_status != 0) as final_price')->where('DATE(created_at) = CURDATE() and employee_id = '.$employee)->order_by("(SELECT employee_username from employees_info where employee_id = orders.employee_id)")->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get("orders")->result();
			elseif($status == "employee") :
				$orderDetails = $this->db->select('id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username, (SELECT territory_name from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)) as territory, (SELECT area_name from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id))) as area, (SELECT region_name from regions_info where id = (SELECT region_id from area_management where id = (SELECT area_id from territory_management where id = (SELECT territory_id from employees_info where employee_id = orders.employee_id)))) as region, employee_id, (SELECT retailer_name from retailers_details where id = orders.retailer_id) as distributor_name, (SELECT retailer_lats from retailers_details where id = orders.retailer_id) as retailer_lats, (SELECT retailer_longs from retailers_details where id = orders.retailer_id) as retailer_longs, retailer_id as distributor_id, within_radius, status, booker_lats, booker_longs,DATE(created_at) as created_at, (SELECT SUM(final_price) from order_contents where order_id = orders.id and item_status != 0) as final_price')->order_by("(SELECT employee_username from employees_info where employee_id = orders.employee_id)")->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get("orders")->result();
			endif;
			$orderStats = array('Pending'=>$this->db->select('count(*) as pendingOrders')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'")) and (status is null or status = "pending" or status = "")')->get('orders')->row()->pendingOrders,'Total'=>$this->db->select('count(*) as totalOrders')->where('employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->totalOrders,'Cancelled'=>$this->db->select('count(*) as cancelledOrders')->where('status = "cancelled" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->cancelledOrders, 'Completed'=>$this->db->select('count(*) as completedOrders')->where('status= "completed" and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->completedOrders, 'Compliance'=>$this->db->select('count(*) as compliance')->where('within_radius = 1 and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->compliance, 'NonCompliance'=>$this->db->select('count(*) as non_compliance')->where('within_radius = 0 and employee_id IN (SELECT employee_id FROM `distributor_assignment` where distributor_id = (SELECT distributor_id from admin_session where session = "'.$this->session->userdata("session").'"))')->get('orders')->row()->non_compliance);
