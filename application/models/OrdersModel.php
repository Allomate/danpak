	...
	public function getCollectiveOrders($status){
		...
		if($this->session->userdata("user_type") == "danpak"){
			...
			if(!$status || $status === "pending"){
				...
				$dates = $this->db->select('DATE(created_at) as date')->where('employee_id IN ('.($reportees.",".$managers).') and (LOWER(status) = "'.$status.'" OR status IS NULL or status = "")')->group_by('DATE(created_at)')->get('orders')->result();

				$emps = $this->db->select('employee_id, (SELECT employee_username from employees_info where employee_id = orders.employee_id) as employee_username')->where('(employee_id IN ('.($reportees.",".$managers).')) and (LOWER(status) = "'.$status.'" OR status IS NULL or status = "")')->group_by('employee_id')->get('orders')->result();
