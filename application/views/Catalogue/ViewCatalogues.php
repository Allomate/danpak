...
<div class="wrapper theme-1-active">
	...
	<div class="page-wrapper">
		<div class="container-fluid">
			...
			<div class="row">
				<div class="col-md-12">
					<div class="box-white p-20"><a href="<?= base_url('Catalogue/CreateCatalogue');?>" class="btn add-emp"><i class="fa fa-plus">
							</i> New Catalogue</a>
									...
									<tbody>
										...
										<tr>
											...
											<td style="max-width: 150px">
												<?= $catalogue->inventory; ?><small class="viewCompleteCatalogueList" id="<?= $catalogue->id; ?>" style="font-weight: bold; cursor: pointer;">(VIEW)</small></td>
											<td>
...
<input type="text" value="<?= base_url('Catalogue/ViewCompleteCatalogueListAjax'); ?>" id="getCatalogueList" hidden>
<input type="text" value="<?= base_url(); ?>" id="baseUrlOnly" hidden>
		...
		$(document).on('click', '.viewCompleteCatalogueList', function () {
			...
			$.ajax({
				...
				success: function (response) {
					...
					$('.modal-body').html(
						'<table class="table" id="catalogueDetailsTable"><thead><tr><th>Thumbnail</th><th>Name</th><th>Unit</th><th>Category</th></tr></thead><tfoot><tr><th>Thumbnail</th><th>Name</th><th>Unit</th><th>Category</th></tr></tfoot><tbody>'
					);
					for (var i = 0; i < response.length; i++) {
						$('#catalogueDetailsTable tbody').append('<tr><td><img src="' + response[i].item_thumbnail +
							'" width="150px" height="auto" /></td><td><a href="' + $('#baseUrlOnly').val() +
							'Inventory/UpdateInventorySku/' + response[i].item_id + '" target="_blank">' + response[i].item_name +
							'</a></td><td>' + response[i].unit_name + '</td><td>' + response[i].category + '</td></tr>');
					}
