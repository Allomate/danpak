...
<script>
	...
	$(document).ready(function () {
		...
		$(document).on('change', '.unitsInContentsDD', function () {
			...
			$.ajax({
				type: 'GET',
				url: '/Orders/GetPriceForThisVariant/' + itemId + '/' + unitIdSelected + '/' + orderId,
		...
		$(document).on('click', '#open_right_sidebar', function () {
			...
			$.ajax({
				type: 'GET',
				url: '/Orders/UpdateOrderAjax/' + thisRef.find('[type="number"]').val(),
				success: function (response) {
	...
	function getUnitsForCurrentitem(orderId) {
		...
		$.ajax({
			type: 'GET',
			url: '/Orders/GetUnits/' + $('#item_id').val() + '/' + orderId,
			success: function (response) {
