...
foreach($objRetsChartData->retail_outlets_data as $data){
    if($counter < (sizeOf($objRetsChartData->retail_outlets_data)-1)){
        $retailerOutletData .= "('" . $data->booking_territory . "', '" . $data->territory . "', '" . $data->total_retailers_added . "', '" . $data->ordering_retailers . "', 'retail_outlets_data', DATE_ADD(NOW(), INTERVAL 11 HOUR)), ";
    }else{
        $retailerOutletData .= "('" . $data->booking_territory . "', '" . $data->territory . "', '" . $data->total_retailers_added . "', '" . $data->ordering_retailers . "', 'retail_outlets_data', DATE_ADD(NOW(), INTERVAL 11 HOUR)) ";
    }
    $counter++;
...
foreach($objRetsChartData->top_10_territories_data as $data){
    if($counter < (sizeOf($objRetsChartData->top_10_territories_data)-1)){
        $top10Territories .= "('" . $data->territory . "', '" . $data->revenue . "', 'top_10_territories_data', DATE_ADD(NOW(), INTERVAL 11 HOUR)),";
    }else{
        $top10Territories .= "('" . $data->territory . "', '" . $data->revenue . "', 'top_10_territories_data', DATE_ADD(NOW(), INTERVAL 11 HOUR)) ";
    }
    $counter++;
...
$sql = "INSERT INTO `dashboard_retailer_temp`(`0_300`, `301_500`, `501_1000`, `1000`, `report_type`, `created_at`) VALUES ('".$result["0_300"]."','".$result["301_500"]."','".$result["501_1000"]."','".$result["1000"]."', 'arpu', DATE_ADD(NOW(), INTERVAL 11 HOUR))";
$stmt = $conn->prepare($sql);
$stmt->execute();
$stmt->close();
